...
export default function Home() {
	...
	function getNewGame() {
		...
		try {
			if (
				selectRef.current.value.includes("kindergarten") ||
				selectRef.current.value.includes("elementary")
			) {
				diff = "very-easy";
			...
			if (
				menuRef.current.value.includes("kindergarten") ||
				menuRef.current.value.includes("elementary")
			) {
				diff = "very-easy";
		...
		axios
			...
			.then((response) => {
				...
				let search_words =
					response.data.search_words.length > 10
						? response.data.search_words.slice(0, 11)
						: response.data.search_words;
				console.log(search_words);
				setSearchWords(search_words);
				let answers =
					response.data.search_words.length > 10
						? Object.fromEntries(
								Object.entries(response.data.answers).filter(
									([key]) => searchWords.includes(key)
								)
						  )
	...
	const onSelectAge = (event) => {
		if (
			event.target.value === "kindergarten" ||
			event.target.value === "elementary"
		) {
			createNewGame();
			setMode("10");
			setIncrease(false);
	...
	function showMenu() {
		return (
			<div className="flex justify-center items-end gap-5">
				<button
					className="bg-green-600 text-white p-2 min-w-[150px] h-14"
	...
	const showAnswers = () => {
		console.info("This button is being clicked!!!");
	...
	return (
		<main className="bg-white flex min-h-fit max-h-full h-full flex-col items-center">
			<div className="game flex flex-col w-fit h-full max-h-full items-center p-5">
				...
				>
					<div
						className={`h-full w-[90%] flex ${
							isNewGame && isGameData
								? ""
					...
					<div
						className={`${
							!showLoading && !isGameData ? "hidden" : ""
						} h-full border-2 border-black flex flex-col w-[250px] p-2`}
					>
						<div className={"flex flex-col w-full gap-4"}>
							...
							<button
								className="bg-[#5300A8] text-white p-2 min-w-[150px] h-14"
								onClick={showAnswers}
							>
								Answers
							</button>
						...
						<div className={`mt-5 `}>
							...
							<select
								...
								onInput={(event) => onSelectAge(event)}
							>
								<option selected value={"kindergarten"}>
									Kindergarten 3-6
								</option>
				...
				<div
					className={`${
						showModal ? "" : "hidden"
					} fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center text-lg`}
				>
